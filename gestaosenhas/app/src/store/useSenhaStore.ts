  ...
  persist(
    (set, get) => ({
      ...
      gerarSenha: (nomePessoa, tipoAtendimento, observacao) => {
        ...
        const prefixo = TIPO_PARA_FILA[tipoAtendimento].toUpperCase();
        ...
        set((s) => ({
          ...
          filas: s.filas.map((f) =>
            f.id === fila.id ? { ...f, senhaAtual: novoNumero } : f
          ),
        }));
      ...
      chamarProxima: () => {
        ...
        set((s) => ({
          senhas: s.senhas.map((sen) =>
            sen.id === proxima.id ? senhaChamada : sen
          ),
          senhaAtualChamada: senhaChamada,
        }));

        // Finaliza senha anterior no histórico
        if (senhaAnterior && senhaAnterior.status === "atendendo") {
          get().finalizarAtendimento(senhaAnterior.id);
        }

        // Após 2 segundos, marca como "atendendo"
        setTimeout(() => {
          set((s) => ({
            senhas: s.senhas.map((sen) =>
              sen.id === proxima.id ? { ...sen, status: "atendendo" } : sen
            ),
      ...
      chamarSenhaEspecifica: (senhaId) => {
        ...
        set((s) => ({
          senhas: s.senhas.map((sen) =>
            sen.id === senhaId ? senhaChamada : sen
          ),
          senhaAtualChamada: senhaChamada,
        }));
        ...
        setTimeout(() => {
          set((s) => ({
            senhas: s.senhas.map((sen) =>
              sen.id === senhaId ? { ...sen, status: "atendendo" } : sen
            ),
            senhaAtualChamada: { ...senhaChamada, status: "atendendo" },
          }));
        }, 3000);
      ...
      finalizarAtendimento: (senhaId) => {
        ...
        set((s) => ({
          senhas: s.senhas.map((sen) =>
            sen.id === senhaId ? senhaFinalizada : sen
          ),
      ...
      getSenhasHoje: () => {
        ...
        return get().senhas.filter(
          (s) => new Date(s.dataCriacao).toDateString() === hoje
        );
      },
