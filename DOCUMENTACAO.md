...
*   **Auto-registro do membro (oficial, sem validação):** somente no dia da atividade, das 00h às 23h59 (janela calculada em `src/lib/attendance.ts` e conferida na action `registerOwnAttendance` em `src/app/(auth)/dashboard/frequencia/actions.ts`), o membro registra presença ou justifica falta (com motivo opcional) pelos botões em `/dashboard/agenda` e, quando há gira hoje sem registro, por um cartão de destaque na home `/dashboard`. Em `/dashboard/frequencia` (lista "Próximas atividades") os botões ficam **sempre ativos**: o membro pode registrar ou corrigir com antecedência (para giras com inscrição prévia, como as de Pedreira) — nesse caso a action é chamada com `advance=true`, que ignora a janela do dia mas bloqueia atividades já passadas. O registro já vale como frequência oficial; `marked_by` guarda o próprio membro. Para atividades passadas, correções ficam com a administração.
