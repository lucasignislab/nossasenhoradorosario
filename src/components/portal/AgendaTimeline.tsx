...
import { EventRowActions } from './EventForm';
 ...
 * As abas mostram o mês vigente + todos os meses que têm atividades cadastradas.
 */
export function AgendaTimeline({ events }: { events: PortalEvent[] }) {
  ...
  return (
    <>
      ...
      {filtered.length === 0 ? (
        <p className="portal-panel__copy">Nenhuma atividade em {monthLabel(activeKey).toLowerCase()}.</p>
      ) : (
